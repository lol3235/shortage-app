...
| 提交 | 类型 | 说明 |
|---|---|---|
| `pending` | feat | 标记到货时**回写企业微信在线表**状态列：`sync.py` 封装 `sheet_get_info`/`sheet_update_range_data`；新增 `writeback.py`（`plan_online_write` 只读定位 + `apply_online_write` 写入，含单表连续/无断层/唯一命中/坐标界内/状态列存在 多重安全闸）；前端加确认弹窗预览将改哪些子表/行/旧值→新值 |
| `pending` | docs | PRD 升 v1.3，新增 §10「在线表写回」，说明权限要求（智能机器人需在线表【编辑】权限，否则 `851003 no authority`）与已知限制 |
