...
## v1.4 · 2026-08-13 · 废弃在线表写回，确认到货回归本地标记

| 提交 | 类型 | 说明 |
|---|---|---|
| 5db3b668fe7dcde1193231cd167c89c606cd6d89 | refactor | 废弃方案 B（网页端写回企业微信在线表）：`app.py` 移除 `import writeback` 及 `preview`/`write_online` 分支，`/api/resolve`、`/api/resolve_text` 仅做本地覆盖；前端确认弹窗改为「确认到货（本地标记）」，明确提示原始在线表需手动修改；`writeback.py` 标注废弃保留 |
| 5db3b668fe7dcde1193231cd167c89c606cd6d89 | docs | PRD §10 改为「已废弃」并说明根因（10 人以上企业模式机器人仅能编辑自己创建的文档，欠料表为成员创建，写入恒报 851003）；同步 CHANGELOG |

**本期核心变化**：因企业微信平台限制，网页端无法改写成员创建的原始在线表。经用户确认，放弃自动写回，恢复为「本地看板标记 + 用户手动改原始表」的轻量模式；代码层写回逻辑保留备查，但不被调用。
