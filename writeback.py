...
原本设计：把本地确认的「已到货 / 已解决」同步写回企业微信在线表对应单元格。

为何废弃（2026-08-13）：
- 企业微信 10 人以上企业模式下，API 模式机器人仅可编辑【自己创建】的文档/表格，
  不会继承授权用户对成员手工创建文档的编辑权。欠料在线表是成员创建的，
  机器人调 sheet_update_range_data 始终返回 851003 no authority（即便已授权「编辑文档」）。
- 经用户确认，放弃网页端直接改写原始表，改为用户手动修改原始表 + 本地标记（方案 A）。
- app.py 已不再 import 本模块，前端也不再调用。保留文件仅作参考，请勿重新启用。
"""
import re
import sync
